...
public class ObserverGuiClient extends javax.swing.JFrame {
    ...
    private void initObserver(String args[]) {
        try {
            ...
            // CAN BE DONE ON IMPL
            this.subjectRI.attach(this.observer);
    ...
    private void getOldMessages(){
        try {
            ArrayList<State> msg = this.observer.subjectRI.getAll();
            int size = msg.size();
            for (int i=0; i<size; i++){
                String m = "[" + msg.get(i).getId() + "]" + msg.get(i).getInfo();
                this.jTextAreaChatHistory.append(m + '\n');
            }
        }catch (RemoteException e){
            e.printStackTrace();
        }
